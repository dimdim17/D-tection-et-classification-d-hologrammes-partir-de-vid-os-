...
def process_folder(input_folder, output_folder, kernel_size=5):
    ...
    for i, file_path in enumerate(filenames):
        ...
        # Détection des zones surexposées (seuil adouci)
        mask = (gray >= 240).astype(np.uint8) * 255

        # Dilatation pour élargir les zones à corriger
        kernel = np.ones((kernel_size, kernel_size), np.uint8)
        processed_mask = cv2.dilate(mask, kernel, iterations=1)

        # Érosion légère pour affiner les bords
...
output_folder = "C:/Users/h09184/Downloads/detection_classification_hologrammes/images/output2"
