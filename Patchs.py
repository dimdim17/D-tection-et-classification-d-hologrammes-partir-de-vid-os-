    ...
    for y in range(0, height, patch_height):
        for x in range(0, width, patch_width):
            if cpt == patch_id:
                ...
                # Vérifier la taille du patch
                if patch.shape[:2] != (patch_height, patch_width):
                    # Étendre le patch en répétant les pixels
...
def get_mosaic(patch_id, frames_path):
    ...
    for root, dirs, files in os.walk(frames_path):
        for file in files:
            mosa.append(get_patch(patch_id, os.path.join(root, file)))
    return mosa
    ...
    for k in range(number_of_patch):
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                if len(mosa) > 0:
                    repeated_patch = patches_array[patch_index % len(mosa)]
                    final_image[i * patch_height: (i + 1) * patch_height, j * patch_width: (j + 1) * patch_width, :] = repeated_patch
        
        mosaics1[labels_mask[k]].append(final_image)
    return mosaics1
...
if __name__ == '__main__':
    all_("C:/Users/h09184/Downloads/detection_classification_hologrammes/images/passport_cropped", 
         "C:/Users/h09184/Downloads/detection_classification_hologrammes/images/Mosaics1")
